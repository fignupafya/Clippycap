...
### Scanning a library
For each enabled `Source`: walk the folder (respecting `recursive`, ignored globs, hidden files per config) → for each file ask each `MediaTypeProvider.detect()` → if a type claims it: look up `HashCache` by `(path, size, mtime)`; if hit, reuse the hash, else compute it via the configured `IdentityStrategy` (Section 7) and store it → find the Asset with that identity hash:
- **found** → ensure this `path` is recorded and `present = true`; refresh metadata if the file changed; emit `AssetUpdated` if anything changed.
- **not found** → create the Asset, extract metadata, enqueue a thumbnail job; emit `AssetAdded`.
Any `AssetPath` not seen this scan → `present = false`. Any Asset whose paths are all absent → it's now **missing** (emit `AssetMissing`); its tags/notes/references stay. A file modified within the last *N* seconds is skipped (OBS may still be writing it). `ScanStarted` / `ScanCompleted` bracket the run; progress is streamed to the UI.
...
**v1 strategy — `CompositeHash`:** compute a cheap **partial hash** first — `sha256(first N KB ‖ last N KB ‖ exact byte size)` (N from config) — and use it as a fast candidate key. Only if two files share a partial hash do we compute the **full BLAKE3** of each to confirm true equality. With a `(path, size, mtime) → hash` cache, a re-scan re-hashes only new or changed files; an initial scan of a few-GB library is seconds. *Why this and not just a full hash everywhere:* large/long recordings make full hashing of an unchanged library wasteful; partial-first removes that cost while the full-hash confirmation removes the (small) partial-hash collision risk. *Why not just a partial hash:* tiny but real collision probability — unacceptable for *identity*.

**Other strategies (pluggable, future):**
- `Blake3FullHash` — simplest, always-full; selectable for users who want it.
...
- **Phase 1 — data & scanning** *(done):* SQLite schema + migrations + repos + FTS5 · scanner: walker + `Blake3IdentityStrategy` + `HashCache` + skip-recent + volume/file-id de-dup · `media_types/video` + the holder-backed ffmpeg adapters (thumbnailer / metadata extractor / video editor) + filename `recorded_at` parsing · `ScanService`, `AssetService` + integration tests (rename/move/missing/merge/copy).
