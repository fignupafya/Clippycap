...
| D18 | **Companion-file Linkers** — a domain-neutral *extract → transform → join → resolve* engine (`app/linking/`) that auto-attaches arbitrary files (demos, scripts, transcripts, RAWs, …) to assets by a user-defined rule. Each rule is a versioned, validated JSON blob (`LinkerDefinition`) stored on a `linkers` row; links live in `asset_attachments`; manual pin/exclude `attachment_overrides` survive every re-run. The matching core is **pure** (no I/O — extraction reads files in `LinkerRunner`, the engine takes contexts), so the whole template/transform/predicate/resolution matrix is unit-tested without the filesystem. Resolution is a real global selection: greedy for keep-all/best-per/quota, **min-cost-flow** for the optimal `best_overall` assignment under per-side caps. See **`LINKERS.md`** for the full design | A real user wanted "attach the demo recorded when this clip happened, so I can find it later"; generalised (per the dev) to *link any file by any rule* — a film editor pairing takes to screenplay `.docx`, a podcaster to transcripts. Matching is fundamentally a **global** problem (two clips competing for one demo; "keep the best 10"; exclusive 1:1), not per-pair — so a dedicated resolve layer was non-negotiable. Pure engine + a thin I/O runner keeps it exhaustively testable | A demo-specific feature (rejected — the dev explicitly wanted it general); per-pair matching only (rejected — breaks the moment candidates compete); a formula/code builder (rejected — must be usable by non-programmers, so power is reached by stacking visual steps, never a code modality) | New field sources / transform steps / predicates / resolution strategies are additive (each a small pure function + a dropdown entry); presets are data; the engine is media-type-agnostic |
...
- **Phase 5 — companion-file Linkers** *(backend done & tested; frontend functional):* the pure `app/linking/` engine (`template`/`values`/`steps`/`extract`/`predicates`/`resolve`/`engine`) + presets · `LinkerRunner` (filesystem bridge + attachment sync + missing-detection + defer-until-enriched) · `LinkerService` (CRUD/preview/run/overrides/reveal/open-with) · migration v11 (`linkers`/`asset_attachments`/`attachment_overrides`/`linker_file_cache`) · `[linkers]` config · `/api/linkers*` + `/api/assets/{id}/attachments` + `/api/attachments/{id}/{reveal,open,open-with}` + overrides routes · 47 engine/integration tests (incl. MCMF optimality vs brute force) · frontend: detail-view **Linked files** panel (reveal / open / open-with / exclude / unpin), **Linkers** page (list + enable toggle + preset gallery + clone + import/export), the **LinkerBuilder** (story-flow scope→files→read-each-side→match-sentence→resolution→actions + visual transform steps + live preview harness + raw-JSON escape hatch). **Open** (per design `LINKERS.md`): by-example highlight-to-capture tokenizer & Flash-Fill inference; the full Contested-view preview; component-partitioned resolution + sort/hash join strategy for very large libraries.
